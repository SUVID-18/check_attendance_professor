...
class AttendanceManagementViewModel {
  ...
  ///```
  factory AttendanceManagementViewModel() =>
      AttendanceManagementViewModel._privateConstructor();
  ...
  Stream<List<AttendanceInformation>>? loadAttendanceDB(String subjectId) {
    ...
    if (currentUser != null) {
      ...
      var db = FirebaseFirestore.instance
          .collection('attendance_history/professor/${currentUser.uid}');
      return db.where('subject_id', isEqualTo: subjectId).snapshots().map(
          (event) => event.docs
              .map((e) => AttendanceInformation.fromJson(
                  documentId: e.id, json: e.data()))
              .toList());
    } else {
      return null;
    }
  ...
  Future<void> editAttendanceInformation(AttendanceInformation studentHistory,
